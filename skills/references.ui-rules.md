...
The checkable design rules. Used by `/design` (per-screen approval loop) and
`/review` (before any ticket touching UI is considered complete).

These are **rules already decided**, not aesthetic opinions. Every one is
...
If everything blurs into an even grey mush, there is no hierarchy. If the
wrong thing stands out, emphasis is misplaced.
